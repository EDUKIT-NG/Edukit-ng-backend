...
export const trackFailedLogin = (email) => {
  const now = Date.now();
  const attempts = failedLoginAttempts.get(email) || { count: 0, lockUntil: null };
  ...
  if (attempts.count >= 5) {
    const newLockoutTime = BASE_LOCKOUT_TIME * Math.pow(LOCKOUT_MULTIPLIER, attempts.count - 5);
    attempts.lockUntil = now + newLockoutTime;
  }

  failedLoginAttempts.set(email, attempts);
  logger.info(`Failed login attempts: ${JSON.stringify(Array.from(failedLoginAttempts), null, 2)}`);
  return { locked: !!attempts.lockUntil, remainingTime: attempts.lockUntil ? (attempts.lockUntil - now) / 1000 : 0 };
};
